 ...
 * w strukturze @p pf istnieje przekierowanie zmieniające @p x w @p num,
 * to numer @p x należy do wyniku wywołania @ref phfwdReverse z numerem @p num.
...
/** @brief Funkcja odwrotna do phfwdGet
 * Wyznacza posortowaną leksykograficznie listę wszystkich takich numerów
 * telefonów i tylko takich numerów telefonów x, że phfwdGet(pf, x) = num.
 * Funkcja ta alokuje strukturę PhoneNumbers, która musi być zwolniona za
 * pomocą funkcji phnumDelete. Wynikiem funkcji jest NULL, gdy nie udało
 * się alokować pamięci. Jeśli podany napis num nie reprezentuje numeru,
 * wynikiem jest pusty ciąg.
 * @param[in] pf
 * @param[in] num
 * @return Wskaźnik na strukturę przechowującą ciąg numerów lub NULL, gdy nie
 *         udało się alokować pamięci.
